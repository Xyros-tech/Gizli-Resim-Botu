...
async def send_image(user, image_path, prize_id, censored=True):
    if not censored:
        image_path = image_path.replace('hidden_img', 'img')
    ...
    if interaction.type == discord.InteractionType.component:
        ...
        if manager.get_winners_count(custom_id) < 3:
            ...
            if res:
                ...
                await send_image(interaction.user, f'img/{img}', custom_id, censored=False)
                await interaction.response.send_message(content="Tebrikler, resmi aldınız!")
            else:
